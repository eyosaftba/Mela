...
import Background_img from "../../assets/images/auth_background_image.webp";
...
function Login() {
  ...
  const LoginWithEmailandPassword = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const { data, error } = await supabase.auth.signInWithPassword(userValues);
  ...
  const usLoggedin = localStorage.getItem("sb-phezxikpdsjliablknoy-auth-token");
  const parsedUser = JSON.parse(usLoggedin);



  useEffect(() => {
    if (location.pathname == "/login" && parsedUser?.access_token) {
      navigate("/");
    }
  });
    ...
    <div className="auth-page">
      ...
      <div className="auth-container">
        <div className="auth-header">
          <h2> Login to your Mela account </h2>
          <p
            style={{
              color: "#6a6a6a",
              fontSize: "13px",
          ...
          >
            Login to your account to access job openings
          </p>
        </div>
